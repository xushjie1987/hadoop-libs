...
public class CompositeService extends AbstractService {
    ...
    /**
     * Policy on shutdown: attempt to close everything (purest) or only try to close started
     * services (which assumes that the service implementations may not handle the stop() operation
     * except when started. Irrespective of this policy, if a child service fails during its init()
     * or start() operations, it will have stop() called on it.
     */
    ...
    /**
     * If the passed object is an instance of {@link Service}, add it to the list of services
     * managed by this {@link CompositeService}
     * 
     * @param object
     ...
     *            flag to say "only start services that are started, not those that are NOTINITED or
     *            INITED.
     * @throws RuntimeException
     *             the first exception raised during the stop process -<i>after all services are
     *             stopped</i>
     */
    private void stop(int numOfServicesStarted,
        ...
        for (int i = numOfServicesStarted - 1; i >= 0; i--) {
            ...
            if (state == STATE.STARTED || (!stopOnlyStartedServices && state == STATE.INITED)) {
                Exception ex = ServiceOperations.stopQuietly(LOG,
                                                             service);
    ...
    /**
     * JVM Shutdown hook for CompositeService which will stop the give CompositeService gracefully
